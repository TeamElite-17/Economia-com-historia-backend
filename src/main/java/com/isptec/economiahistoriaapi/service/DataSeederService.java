...
public class DataSeederService implements ApplicationRunner {
    ...
    private void seedCategories() {
        String[][] cats = {
            ...
            {"Economia Colonial",       "economia-colonial",     "Conteudos sobre o periodo colonial em Angola"},
            {"Financas Pessoais",       "financas-pessoais",     "Gestao de dinheiro e financas para o dia a dia"},
            {"Economia Global",         "economia-global",       "Economia mundial e suas influencias em Angola"},
    ...
    private void seedContentItems() {
        if (contentItemRepository.count() > 3) return;
    ...
    private void seedForumData() {
        if (!forumThreadService.getAllThreads().isEmpty()) return;

        User admin = userRepository.findByEmail("admin@economia.ao").orElse(null);

        ForumThread t1 = forumThreadService.createThread(
                ForumThread.builder().title("Como poupar dinheiro em Angola com a inflacao atual?").build());
